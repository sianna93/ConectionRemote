...
public class Usuario extends _Default {
    ...
    public void salvar(){
        String comando = "";
        if (this.getId() == -1){
            comando = String.format("INSERT INTO public.usuario (nome, email, telefone) VALUES ('%s','%s','%s');",
                    this.getNome(),this.getEmail(),this.getTelefone());
        }else{
            comando = String.format("UPDATE public.usuario SET nome = '%s', email = '%s', telefone = '%s' WHERE id = %d;",
    ...
    public void apagar(){
        String comando = String.format("DELETE FROM public.usuario WHERE id = %d;",this.getId());
        DB db = new DB();
        db.execute(comando);
        this._mensagem = db._mensagem;
        this._status = db._status;
